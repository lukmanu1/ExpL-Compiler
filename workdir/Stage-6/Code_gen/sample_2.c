...
void initialize_system_call(FILE* fptr){

    fprintf(fptr, "INIT_FUNCTION:\n");
    ...
    fprintf(fptr, "INIT_LOOP:\n");
    fprintf(fptr, "MOV R%d, R%d\n", temp_reg, addr_reg);
    fprintf(fptr, "LT R%d, %d\n", temp_reg, 2040);
    fprintf(fptr, "JZ R%d, INIT_END\n", temp_reg);
    fprintf(fptr, "MOV R%d, R%d\n", temp_reg, addr_reg);
    fprintf(fptr, "ADD R%d, %d\n", temp_reg, 8);
    fprintf(fptr, "MOV [R%d], R%d\n", addr_reg, temp_reg);
    fprintf(fptr, "ADD R%d, 8\n", addr_reg);
    fprintf(fptr, "JMP INIT_LOOP\n");
    fprintf(fptr, "INIT_END:\n");
...
int code_gen_init_function_call(FILE* fptr){
    ...
    // Call the function
    fprintf(fptr, "CALL INIT_FUNCTION\n");
...
void free_system_call(FILE* fptr){

    fprintf(fptr, "FREE_FUNCTION:\n");
    ...
    fprintf(fptr, "ADD R%d, -3\n", reg);
    fprintf(fptr, "MOV R%d, [R%d]\n", reg, reg);

    fprintf(fptr, "MOV [R%d], [1024]\n", reg);
    fprintf(fptr, "MOV [1024], R%d\n", reg);

    // according to my implementation, i am not expecting any return value for free, but according to the document i think i should return NULL, which i am not sure, if that the case then update the code according to that.
    int ret_addr = get_reg();
    fprintf(fptr, "MOV R%d, BP\n", ret_addr);
    fprintf(fptr, "ADD R%d, -2\n", ret_addr);
    fprintf(fptr, "MOV [R%d], 1000\n", ret_addr); // here i added to just to indicate that 1000 is mirco for NULL VALUE.
...
int code_gen_free_function_call(FILE* fptr, ast_node* calle){
    ...
    fprintf(fptr, "CALL FREE_FUNCTION\n");
...
void alloc_system_call(FILE* fptr){

    fprintf(fptr, "ALLOC_FUNCTION:\n");
    int reg = get_reg();
    fprintf(fptr, "MOV R%d, [1024]\n", reg);
    fprintf(fptr, "MOV[1024], [R%d]\n", reg);
...
// int code_gen_alloc_function_call(FILE* fptr, ast_node* calle){
//     int current_reg_val = reg + 1;
//     push_registers(fptr);
...
int code_gen_alloc(FILE* fptr, ast_node* node){
...
int codeGen(FILE *fptr, ast_node *node)
{
    ...
    case NODE_TYPE_INIT_FUNCT_CALL:
        code_gen_init_function_call(fptr);
        free_reg(); // return valued is not used anywhere what is why this free, function return register holding the return value.
        return -1;

    case NODE_TYPE_FREE_FUNCT_CALL:
        return code_gen_free_function_call(fptr, node); // HERE I AM AVOIDING free_reg function, i should add here one free_reg function, code_gen_free_funciton_call function is returning a Reg holding 1000, which is not going to be used any-where in the code according to my grammar.

    case NODE_TYPE_ALLOC_FUNCT_CALL:
        return code_gen_alloc_function_call(fptr, node);
