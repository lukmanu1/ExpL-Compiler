...
void initialise_vtable(FILE* fptr, Classtable* head){
    Classtable* temp = head;
    int start_addr = 4096;
    while(temp){
        int base = start_addr + temp->Class_index * 8;

        Memberfunclist* fields = temp->Vfuncptr;
        int index = 0;

        while(fields){

            fprintf(fptr, "MOV [%d], %s%d\n", base + index, fields->Name, fields->Flabel);
            index++;
            fields = fields->Next;
...
void point_vtable(FILE* fptr, gst_node* head){
    gst_node* temp = head;
    ...
    while(temp){
        ...
        int addr = temp->binding + 1;
        int v_addr = 4096 + 8 * temp->Ctype->Class_index;
...
int code_gen_main_function(FILE *fptr)
{
    ...
    fprintf(fptr, "MAIN:\n");
    initialise_vtable(fptr, ClassTable);
    point_vtable(fptr, head);
    fprintf(fptr, "PUSH BP\n");
    fprintf(fptr, "MOV BP, SP\n");
...
int code_gen_class_function_call(FILE* fptr, ast_node* calle){

    int current_reg_val = reg + 1;
    push_registers(fptr);

    push_arguments(fptr, calle->ptr2->arglist);

    int addr_reg = resolve_address(fptr, calle->ptr1);
    int v_addr = get_reg();
    ...
    fprintf(fptr, "MOV R%d, [R%d]\n", obj_addr, obj_addr);
    fprintf(fptr, "ADD R%d, %d\n", obj_addr, field_list->Funcposition);
    fprintf(fptr, "MOV R%d, [R%d]\n", obj_addr, obj_addr);
    free_reg();
    ...
    ast_node *t = calle->ptr2->arglist;
    int argCount = 1; // here i made it as one, because i want to do one extra pop for v_table pointer
    while (t)
    {
        argCount++;
        t = t->arglist;
...
{
    if(node->nodetype == NODE_TYPE_SELF){
        int addr_reg = get_reg();
        fprintf(fptr, "MOV R%d, BP\n", addr_reg);
        fprintf(fptr, "ADD R%d, -4\n", addr_reg);
        return addr_reg;
    }
    else if (node->nodetype == NODE_TYPE_ID)
    {
        int addr_reg = get_reg();
        if (node->Lentry)
        {
    ...
    else if (node->nodetype == NODE_TYPE_TUPLE)
    {
        ...
        fprintf(fptr, "MOV R%d, [R%d]\n", addr_reg, addr_reg);
        if(node->ptr1->Ctype == NULL )field_offset = find_field_index(node->ptr1->type->fields, node->ptr2->name);
        else field_offset = find_class_field_index(node->ptr1->Ctype->Memberfield, node->ptr2->name);
...
{
    if(node->ptr1->Ctype != NULL && node->ptr2->nodetype == NODE_TYPE_ALLOC_FUNCT_CALL){
        int right = codeGen(fptr, node->ptr2);
        int left_addr = resolve_address(fptr, node->ptr1);
        fprintf(fptr, "MOV [R%d], R%d\n", left_addr, right);
        fprintf(fptr, "ADD R%d, 1\n", left_addr);
        int addr = 4096 + 8 * node->ptr2->Ctype->Class_index;
        fprintf(fptr, "MOV [R%d], %d\n", left_addr, addr);
    }
    else if(node->ptr1->Ctype != NULL && node->ptr2->Ctype != NULL){

        int left_addr = resolve_address(fptr, node->ptr1);
        int right_addr = resolve_address(fptr, node->ptr2);
...
int codeGen(FILE *fptr, ast_node *node)
{
    ...
    case NODE_TYPE_CLASS_FUNCTION_CALL:
        return code_gen_class_function_call(fptr, node);
