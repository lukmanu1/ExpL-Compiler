...
int isSubclass(Classtable *child, Classtable *parent) {
    while (child != NULL) {
        if (child == parent)   // same class or reached parent match
            return 1;
...
ast_node* create_new_function_call_node(ast_node* left, ast_node* right){
    ...
    Classtable* cptr = CLookup(right->name);    // class in NEW()
    if (cptr == NULL) {
        printf("Class Name : %s\n", right->name);
    ...
    // Updated polymorphic check
    if (left->Ctype != cptr) {
        if (!isSubclass(cptr, left->Ctype)) {
            printf("Type Error: Cannot assign object of class '%s' to reference of class '%s'\n",
                   cptr->Name, left->Ctype->Name);
            yyerror("Invalid class allocation\n");
        }
    }
...
ast_node* create_assign_node(ast_node* left, ast_node* right){
    ...
    // --------------- CASE: assignment of NULL -------------------
    
    if(right->nodetype == NODE_TYPE_NULL){
                  
        // primitive variable cannot receive NULL
        // primitive => Ctype == NULL && type != NULL && fields == NULL
        if(left->Ctype == NULL && left->type != NULL && left->type->fields == NULL){
            yyerror("Invalid: Cannot assign NULL to primitive variable");
        }

        // user-defined struct (fields != NULL) OR class (Ctype != NULL) can accept null
        union Constant value;
    ...
    // --------------- CASE: Both sides are primitive types -------------------
    if(left->Ctype == NULL && right->Ctype == NULL){

        // primitive vs primitive
        if(left->type != NULL && right->type != NULL &&
           left->type->fields == NULL && right->type->fields == NULL){
           
            if(strcmp(left->type->name, right->type->name) != 0){
                yyerror("Primitive Type Mismatch in Assignment");
            }
        }
        // user-defined struct types
            ...
            if(strcmp(left->type->name, right->type->name) != 0){
                yyerror("Tuple/User-defined type mismatch in assignment");
            }
        }
        // mixed struct/primitive
        else {
            yyerror("Invalid assignment: struct ↔ primitive mismatch");
        }
    }

    // --------------- CASE: Both sides are class variables -------------------
    else if(left->Ctype != NULL && right->Ctype != NULL){

        
        
        if(left->Ctype != right->Ctype){
            if(!isSubclass(right->Ctype, left->Ctype)){
                printf("Semantic Error: Cannot assign object '%s' to variable '%s'\n",
                        right->Ctype->Name, left->Ctype->Name);
                yyerror("Subclass / inheritance assignment violation");
            }
        }
    }

    // --------------- CASE: Mixed class and primitive/struct -------------------
    else {
        yyerror("Type Mismatch: Cannot assign primitive/user-type to class or vice versa");
    }
