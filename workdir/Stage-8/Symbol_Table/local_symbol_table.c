...
lst_node* create_lst_node(char* name, table_type* type, Classtable* Ctype){
...
void lst_install(char *name, table_type* type, Classtable* Ctype){
   ...
   lst_node* temp = lst_look_up(name);
    ...
    lst_node* new_node = create_lst_node(name, type, Ctype);
...
void add_param_list_to_lst(p_node* param_list){
    ...
    p_node* temp = param_list;
    while(temp){
        lst_install(temp->name, temp->type, NULL);
        temp = temp->next;
    }
    
}

void insert_self(Classtable* class){
    lst_binding = -4;
    lst_install("self", NULL, class);
}
